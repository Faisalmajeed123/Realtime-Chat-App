import { StyleSheet, Text, View, Image, TextInput, Pressable, KeyboardAvoidingView, ScrollView, Alert } from 'react-native'
...
const Signup = () => {
  ...
  const onSubmit =async() => {
    if (!NameRef.current || !EmailRef.current || !PasswordRef.current || !ImageRef.current) {
    ...
    <KeyboardAvoidingView>
      <ScrollView>
        <View className=' justify-center items-center gap-2 p-1 bg-white'>
          <Image className='' style={{ height: hp(30), width: wp(60) }} resizeMode='contain' source={require('../Assets/login.png')} />
          ...
          <View className='gap-4'>
            <View className='border rounded-2xl' style={{ height: hp(7), width: wp(90), backgroundColor: '#E7E7E7' }}>
              <View className='flex-row items-center px-2 gap-1'>
                ...
                <TextInput className='text-black ' placeholder='Name' placeholderTextColor={'gray'} onChangeText={value => NameRef.current = value}/>
              </View>
            </View>
            <View className='border rounded-2xl' style={{ height: hp(7), width: wp(90), backgroundColor: '#E7E7E7' }}>
              <View className='flex-row items-center px-2 gap-1'>
            ...
            <View className='border rounded-2xl' style={{ height: hp(7), width: wp(90), backgroundColor: '#E7E7E7' }}>
              <View className='flex-row items-center px-2 gap-1'>
                ...
                <TextInput className='text-black ' placeholder='Password'  placeholderTextColor={'gray'} secureTextEntry onChangeText={value => PasswordRef.current = value} />
              </View>
            </View>
            <View className='border rounded-2xl' style={{ height: hp(7), width: wp(90), backgroundColor: '#E7E7E7' }}>
              <View className='flex-row items-center px-2 gap-1'>
                <Ionicons name='image-outline' size={28} color={'gray'} />
                <TextInput className='text-black ' placeholder='Image URl' placeholderTextColor={'gray'} onChangeText={value => ImageRef.current = value} />
              </View>
            </View>
            ...
            <View className='flex-row items-center justify-center'>
              <Text className='font-medium text-black'>Already have an account?  </Text>
              <Pressable className='' onPress={() => navigation.navigate('signin')}>
